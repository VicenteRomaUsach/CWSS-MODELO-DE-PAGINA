    ...
    list.innerHTML = projects.map(function (project) {
      return `
        <article class="admin-project-row">
          ...
          <div><h3>${escapeHtml(projectDisplayName(project))}</h3><p class="admin-project-row__meta">Cliente: ${escapeHtml(projectConstructor(project))} · Año: ${escapeHtml(project.year || "Por confirmar")} · Productos: ${escapeHtml(projectProducts(project).join(", "))}</p></div>
